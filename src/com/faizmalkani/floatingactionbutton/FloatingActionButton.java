...
public class FloatingActionButton extends View {
   ...
   */
  private final float        mYDisplayed      = -1;
  /**
  ...
  public FloatingActionButton( Context context , AttributeSet attrs ,
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) setLayerType(
 ...
 */}
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    if (event.getAction() == MotionEvent.ACTION_UP) {
      ...
      rect = new Rect(mLeftDisplayed, mTopDisplayed, mRightDisplayed,
          mBottomDisplayed);
    }
    if (event.getAction() == MotionEvent.ACTION_MOVE) {
      if (!rect.contains(mLeftDisplayed + (int) event.getX(), mTopDisplayed
          + (int) event.getY())) {
        color = mColor;
      }
    }
