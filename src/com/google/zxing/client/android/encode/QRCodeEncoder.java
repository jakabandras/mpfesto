...
import com.andrewsoft.mpfesto.R;
...
final class QRCodeEncoder {
  ...
  private void encodeFromTextExtras(Intent intent) throws WriterException {
    ...
    if (theContents == null || theContents.isEmpty()) { throw new WriterException(
        "Empty EXTRA_TEXT"); }
  ...
  private void encodeFromStreamExtra(Intent intent) throws WriterException {
    ...
    if (bundle == null) { throw new WriterException("No extras"); }
    Uri uri = bundle.getParcelable(Intent.EXTRA_STREAM);
    if (uri == null) { throw new WriterException("No EXTRA_STREAM"); }
    byte[] vcard;
    String vcardString;
    InputStream stream = null;
    try {
      stream = activity.getContentResolver().openInputStream(uri);
      if (stream == null) { throw new WriterException("Can't open stream for "
          + uri); }
    ...
    if (!(parsedResult instanceof AddressBookParsedResult)) { throw new WriterException(
        "Result was not an address"); }
    encodeQRCodeContents((AddressBookParsedResult) parsedResult);
    if (contents == null || contents.isEmpty()) { throw new WriterException(
    ...
    case Contents.Type.CONTACT:
      ...
      if (contactBundle != null) {
        ...
        String[] encoded = encoder.encode(Collections.singletonList(name),
            organization, Collections.singletonList(address), phones,
  ...
  private void encodeQRCodeContents(AddressBookParsedResult contact) {
    ...
    String[] encoded = encoder.encode(toList(contact.getNames()),
        ...
        toList(contact.getPhoneNumbers()), null, toList(contact.getEmails()),
